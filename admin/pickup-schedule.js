...
function initPickupSchedule(p) {
  ...
  function paint() {
    ...
    for (const date of dates) {
      ...
      const label=document.createElement('span');label.textContent=date+(holidays.includes(date)?'（休業日も受付）':'');
  ...
  add.onclick=()=>{
    ...
    const isClosed = (state.closedWeekdays||[]).includes(weekday) || state.closedDates?.has(date);
    if(isClosed) {
      if(!confirm(`${date}は休業日に設定されています。この商品だけ受け付けますか？\nこの商品の受付停止日は優先されます。`))return;
      holidays.push(date);
    }
